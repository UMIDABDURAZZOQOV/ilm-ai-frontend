  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      <header className="h-14 shrink-0 border-b border-slate-200 dark:border-slate-800 flex items-center px-4 sm:px-6 gap-4">
        ...
        {/* Reading guide — a line reader, available on every question. */}
        <button
          onClick={() => setShowLineReader((v) => !v)}
          className={`flex flex-col items-center px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-colors ${
            showLineReader ? "bg-sky-100 text-sky-700 dark:bg-sky-900/40 dark:text-sky-300" : "hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300"
          }`}
          title="Reading guide (line reader)"
        >
      ...
      <main className="flex-1 overflow-y-auto">
        <div className={`mx-auto p-4 sm:p-8 ${isLong ? "max-w-6xl lg:grid lg:grid-cols-2 lg:gap-10" : "max-w-3xl"}`}>
          <div className={isLong ? "lg:border-r lg:border-slate-200 lg:dark:border-slate-800 lg:pr-10 mb-6 lg:mb-0" : "mb-6"}>
            <Annotatable storageKey={`${session?.session_id}-${question.id}`}>
